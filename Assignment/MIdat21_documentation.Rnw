...
\section*{Data}
The \textbf{MIdat21} data comprise data of 1016 liver transplant patients
...
\texttt{center} & study center identifier\\
\texttt{isc\_time} & ischemic time (time the organ was outside the body)\\
\texttt{age\_don} & age of the donor\\
\texttt{bg\_match} & are the blood groups of donor and recipient equal?\\
...
knit2pdf("Assignment/MIdat21_documentation.Rnw",
         output = "Assignment/instructions/MIdat21_documentation.tex")
file.remove("Assignment/instructions/MIdat21_documentation.tex")
@
\end{document}
