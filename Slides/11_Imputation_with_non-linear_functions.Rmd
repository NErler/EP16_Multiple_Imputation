...
To relax the assumption of linear associations even more, we could introduce
additional interactions with the outcome.
...
Alternatively, we could impute all non-linear terms and interactions passively:
\small
```{r}
...
```{r eval = TRUE, ref.label="JointAI_nonlin_trace", fig.width = 7, fig.height = 3.8, echo = FALSE}
```
 ...
 * simultaneous analysis & imputation instead of 3 steps
...
Imputation with non-linear associations \button{https://nerler.github.io/EP16_Multiple_Imputation/practical/minonlin/EP16_MInonlin.html}{html}
\end{block}
\end{column}
\end{columns}
\end{center}
