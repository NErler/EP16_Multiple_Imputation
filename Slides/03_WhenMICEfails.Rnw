...
\put(0, 60){
    ...
    \textcolor{black!50}{\bmath w} &=& \textcolor{black!50}{\bmath X_c \bmath{\beta}_w}
                                   &+& \textcolor{black!50}{\bmath{ Z_w} \,\bmath{ b_w}}
                                   &+& \textcolor{black!50}{\bmath{ \varepsilon_w}}\\
...
\item \blue{Non-linear associations} or interactions involving incomplete covariates
...
ggplot(plot_nonlin, aes(x = L1, y = coef)) +
  ...
  facet_wrap("var", scales = 'free') +
  geom_polygon(data = polyDF, aes(x = x, y = y), fill = 'blue', alpha = 0.3) +
...
(1: repeated measurements, 2: baseline covariates), and ordered the same way
...
jomo_long <- myjomo.lmer(formula = y ~ x1 + x2 + x3 + x4 +
                         time + I(time^2) + (time|id),
                       data = DFexlong2[, names(lvl)], level = lvl)

jomo_longMCMC <- myjomo.lmer.MCMCchain(formula = y ~ x1 + x2 + x3 + x4 +
                                         time + I(time^2) + (time|id),
...
ggplot(plot_long[plot_long$L1 != "mice_naive", ], aes(x = L1, y = coef)) +
  ...
  geom_polygon(data = polyDF, aes(x = pmin(x, 3.5), y = y), fill = 'blue', alpha = 0.3) +
...
\begin{itemize}
\item MICE requires congenial \& compatible imputation models to work well.
\item When this is not the case, (naive) use of MICE can lead to biased results.
\item Common settings that require special attention are
      \begin{itemize}
      \item non-linear functional forms \& interaction terms
      \item longitudinal data
      \item survival data
      \end{itemize}
\item When using the package \textbf{mice}, there are choices that can reduce bias
      \begin{itemize}
      ...
      \item special 2-level imputation methods are available for longitudinal data
      \item The Nelson-Aalen estimator can be used instead of the time variable
            for imputing survival data when effects are not too large.
      \end{itemize}
\framebreak
\item Generally, problems are more severe when
      \begin{itemize}
      \item proportions of missing values are large,
      \item effect sizes are large,
      \item little other covariate information is available.
      \end{itemize}
      (Note that in the examples we had all of the above.)
\item In settings where MICE may not provide valid imputations,
      alternative approaches are available and should be considered.
\item R packages that provide such alternative approaches are for example:
      \begin{itemize}
      \item JointAI (non-linear \& longitudinal)
      \item smcfcs (non-linear \& survival)
      \item jomo (non-linear, longitudinal \& survival)
      \end{itemize}
\item These packages are very young.
      \begin{itemize}
      \item Hence, they may still have some problems.\\
            \blue{\ding{225}} Use them carefully! (and email the maintainer about problems)
      \item They are under active development, so resolutions of bugs and
