...
# # x5 <- rnorm(length(unlist(time)), 34, 6)
# # x2_orig <- x2
# # x3_orig <- x3
# # x4_orig <- x4
# # x5_orig <- x5
#
# # x2[c(18, sample.int(N, size = 0.3 * N))] <- NA
# # x3[sample.int(N, size = 0.3 * N)] <- NA
# # x4[sample.int(N, size = 0.3 * N)] <- NA
# # x5[sample.int(length(unlist(time)),
#               # size = 0.3 * length(unlist(time)))] <- NA
#
...
#                  # x5 = x5,
#                  # x2_orig = rep(x2_orig, sapply(time, length)),
#                  # x3_orig = rep(x3_orig, sapply(time, length)),
#                  # x4_orig = rep(x4_orig, sapply(time, length))
#                  # x5_orig = x5_orig
#                  )
#
...
#            gestbir = 0.3)
#
# fmla <- ~ ns(time, df = 3) + x1 + x2 + x3 s+ x4
...
\begin{columns}
\begin{column}{0.5\linewidth}
<<trajectories_fit, echo = F, fig.with = 6, fig.height = 4>>=
p +
  geom_abline(data = coefDF,
              aes(intercept = `(Intercept)`, slope = age,
                                 color = factor(id)), lty = 2, lwd = 1)
@
\end{column}
\begin{column}{0.5\linewidth}
\begin{itemize}
\item ignore the outcome
\item use only the baseline outcome
\item summarize the \blue{individual profiles} using
  \begin{itemize}
...
ID & $X_1$      & $X_2$      & $X_3$      & $X_4$      & \blue{$\mathbf{b_0}$}\\\hline
1  & \checkmark & NA         & \checkmark & \checkmark & \checkmark \\
2  & NA         & \checkmark & \checkmark & \checkmark & \checkmark\\
3  & \checkmark & NA         & \checkmark & NA         & \checkmark\\
\vdots  & \checkmark & \checkmark & NA         & \checkmark & \vdots
\end{tabular}
}
\vfill
\end{column}
...
\begin{columns}
\begin{column}{0.5\linewidth}
<<trajectories_ri_old, echo = F, fig.with = 6, fig.height = 4>>=
p2 <- p +
  geom_point(data = coefDF, aes(x = 0, y = `(Intercept)`, fill = factor(id)), size = 8,
             shape = 21, alpha = 0.5) +
  geom_abline(data = coefDF,
              aes(intercept = `(Intercept)`, slope = age,
                                 color = factor(id)), lty = 2, lwd = 1) +
...
\begin{columns}
\begin{column}{0.5\linewidth}
<<trajectories_ris_old, echo = F, fig.width = 6, fig.height = 4>>=
starttri <- c(1, 1, 2, 3, 1)
endtri <- starttri + 4

polyDF <- data.frame(x = c(starttri, endtri, endtri),
                     y = c(rep(coefDF$`(Intercept)` + starttri*coefDF$age, 2),
                           coefDF$`(Intercept)` + coefDF$age * endtri
                     ),
                     id = rep(unique(DF$id), 3)
)

p2 + geom_polygon(data = polyDF, aes(x = x, y = y, group = factor(id),
                                     fill = factor(id)),
                  alpha = 0.5)
...
\begin{itemize}
   ...
   \item summarize the \blue{individual profiles} using
         \begin{itemize}
         \item random intercept
         \item random intercept \& slope
         \end{itemize}
   \end{itemize}
   \vfill
   \scalebox{0.8}{
      \begin{tabular}{c|ccccccc}
      ID & $X_1$      & $X_2$      & $X_3$      & $X_4$      & \blue{$\mathbf{b_0}$} & \blue{$\mathbf{b_1}$}\\\hline
      1  & \checkmark & NA         & \checkmark & \checkmark & \checkmark & \checkmark\\
      2  & NA         & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark\\
      3  & \checkmark & NA         & \checkmark & NA         & \checkmark & \checkmark\\
 \vdots  & \checkmark & \checkmark & NA         & \checkmark & \vdots     & \vdots
      \end{tabular}
   }
\end{column}
\end{columns}
