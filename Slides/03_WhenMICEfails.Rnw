...
################################################
gammas <- c("(Intercept)" = -5.7296, "Group" = 2.4092, x = -2) # coefficients for baseline covariates
...
survdat_orig$group <- factor(survdat_orig$group + 1)
...
cox01 <- with(impsurv_01, coxph(Surv(Time, as.numeric(event)) ~ x + group))
...
ggplot(plotcox, aes(x = meth, y = est)) +
  ...
  facet_wrap("var", scales = 'free',
             labeller = labeller(var = c("group2" = "binary covariate",
...
\blue{JointAI} is available from CRAN and GitHub
<<eval = F>>=
...
``substantive model compatible fully conditional specification'', a \blue{hybrid
...
with \blue{extensions to assure compatibility} between analysis and imputation models.

\bigskip

\textbf{jomo} (version 2.5-2) can handle
\begin{itemize}
\item linear regression,
\item generalized linear regression,
\item linear mixed models,
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
As an example, we will impute the second (unbalanced) longitudinal (simulated)
data example from above. There are
\begin{itemize}
\item $x1$ (complete)
...
# adjust imputation method and predictor matrix
...
models <- with(imp, lmer(y ~ x1 + x2 + x3 + x4 + time + I(time^2) + (time|id)))
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
Currently, there is only limited documentation and examples that show how to
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
The longitudinal example data can be analysed in \textbf{JointAI} with the following
syntax:
<<JointAI_long, eval = F>>=
library(JointAI)
...
matplot(t(jomo_longMCMC$collectbeta[1, , ]), type = 'l')
matplot(t(jomo_longMCMC$collectomega[1, , ]), type = 'l')
...
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<echo = F, figwidth = 6, fig.height = 4.5>>=
mod_long2 <- with(DFexlong2_orig, lmer(y ~ x1 + x3 + x2 + x4 + time + I(time^2) +
                                         (time|id)
                                       ))
...
plot_long <- plot_long[!plot_long$var %in% grep("ns(time", plot_long$var, value = T, fixed = T), ]
...
polyDF <- polyDF[!polyDF$var %in% grep("ns(time", polyDF$var, value = T, fixed = T), ]
...
When \blue{covariate effects and cumulative incidences are rather small},
...
However, in practice $H_0(T)$ is usually not known.
\end{frame}
...
Two main ideas:
\begin{itemize}
\item if covariate effects $\beta_x$ and $\beta_z$ are small, $H_0(t)\approx H(t)$,
      which can be approximated by the Nelson-Aalen estimator
\item estimate $H_0(T)$ in an additional step inside the MICE procedure by fitting
      a Cox model on the imputed data
\end{itemize}
Note that neither of these approaches takes into account uncertainty about $H_0(t)$
(but impact is likely to be small)

\bigskip

Based on results from their simulation study, White et al. conclude that using
$Z$, $D$ and the Nelson-Aalen estimator $\hat H(T)$ as predictors for the
imputation of $X$ worked best.

\bigskip

However, some bias towards the null should be expected when covariates have large
effects.
\end{frame}
...
survimp <- mice(survdat, maxit = 10)
cox_mice <- with(survimp, coxph(Surv(Time, as.numeric(event)) ~ x + group))
@
\end{frame}
...
smcfcs_surv <- smcfcs(originaldata = survdat, smtype = "coxph",
                      smformula = "Surv(Time, event) ~ x + group",
                      method = c("", "", "logreg", "norm"),
                      numit = 20)

# check convergence, analogue to example with non-linear effect

impobj_smcfcs_surv <- mitools::imputationList(smcfcs_surv$impDatasets)
models_smcfcs_surv <- with(impobj_smcfcs_surv, coxph(Surv(Time, event)) ~ x + group)
res_smcfcs_surv <- summary(mitools::MIcombine(models_smcfcs_surv))
