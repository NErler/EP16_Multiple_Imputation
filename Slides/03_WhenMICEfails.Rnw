...
\begin{columns}
\begin{column}{0.7\linewidth}
<<echo = F, cache = T, warning=FALSE>>=
impwide <- mice(DFexlongwide, printFlag = F, maxit = 10)
@

<<fig.height = 4, fig.width = 5, echo = F, message = F, warning=FALSE>>=
...
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
\blue{Naive use of MICE}: $T$ and $D$ are treated just like any other variable.
...
\item<4-> \blue{JointAI} works \blue{fully Bayesian}
      ...
      \item \ding{225} results from the analysis model of interest are
obtained directly
      \end{itemize}
\end{itemize}
\end{frame}
...
\item imputed as if it was just another variable.
\end{itemize}
...
\begin{block}{}
\centering\url{https://nerler.github.io/EP16_Multiple_Imputation/practical/minonlin/MInonlin.html}
\end{block}
\end{frame}
...
or find the \textbf{html version} of the practical here:
\begin{block}{}
\centering\url{https://nerler.github.io/EP16_Multiple_Imputation/practical/milong/MIlong.html}
\end{block}
\end{frame}
...
res_mice_surv <- summary(pool(cox_mice, dfcom = 3), conf.int = TRUE)
@
...
res_survreg <- list(mice_naive = summary(pool(mice_naive_wb, dfcom = 99999),
                                         conf.int = TRUE)[names(coef(JointAI_survreg)),
                                                          c('estimate', '2.5 %', '97.5 %')],
                    mice = summary(pool(mice_wb), conf.int = TRUE)[names( coef(JointAI_survreg)),
                                                                   c("estimate", "2.5 %", "97.5 %")],
...
res_cox <- list(mice_naive = summary(pool(mice_naive_cox),
...
\begin{frame}
<<echo = FALSE, fig.width = 6, fig.height = 2.5, size = 'small'>>=
...
or find the \textbf{html version} of the practical here:
\begin{block}{}
\centering\url{https://nerler.github.io/EP16_Multiple_Imputation/practical/misurv/MIsurv.html}
\end{block}
\end{frame}
