...
$$ p(\mathbf y, \mathbf x_1,\ldots, \mathbf x_p \mid \mathbf X_c, \boldsymbol\theta,
     \mathbf b) \sim N(\mathbf \mu, \mathbf \Sigma)$$
...
$$\text{e.g.: } \mathbf x_k \text{ binary }
  \rightarrow \text{ latent } \mathbf{\hat x}_k \text{ is standard normal: }
  \left\{\begin{array}{c} \mathbf x_k = 1\\ \mathbf x_k = 0\end{array}\right.
  \text{ if } \begin{array}{c} \mathbf{\hat x_k}\geq 0\\ \mathbf{\hat x_k} < 0\end{array}
$$
...
\put(3, 60){
    ...
    \mathbf{\hat x}_1 &=& \mathbf X_c \boldsymbol{\beta}_{x_1} &+& \phantom{\mathbf Z_w\,} \boldsymbol{ \varepsilon_{x_1}}      &&\\[-1ex]
               & \vdots&                                & & \phantom{\mathbf Z_w\;} \vdots                                &&\\[-1ex]
    \mathbf{\hat x}_p &=& \mathbf X_c \boldsymbol{\beta}_{x_p} &+& \phantom{\mathbf Z_w\,} \boldsymbol{ \varepsilon_{x_p}}      &&
    \end{array}
    $
    }
}
