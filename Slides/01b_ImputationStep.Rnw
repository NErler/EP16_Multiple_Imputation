...
In the \blue{Bayesian framework}, \blue{everything unknown} or unobserved is considered as
a \blue{random variable}.
Here, this includes for example regression coefficients $\bmath\beta$, residual variance
$\sigma^2$ and missing values $\bmath y_{mis}$ and $\bmath X_{mis}$.

\bigskip

Random variables have a \blue{probability distribution}. The \blue{expectation}
of that distribution quantifies where which \blue{values} of the random
variable are \blue{most likely}, the \blue{variance} is a measure of the
\blue{uncertainty} about the values.

\bigskip

In \blue{Bayesian imputation}, the \blue{information obtained from the observed data}
is used to \blue{estimate the probability distributions} for the missing values and unknown
parameters, and values are \blue{imputed by draws} from that posterior (= after having seen the data)
distribution.
\end{frame}
...
usually a \blue{regression model} is used, that depends on the unknown coefficients $\bmath \beta$.
$$\mathbb E(\bmath y_{mis}\mid \bmath X, \bmath \beta) = f(\bmath X_{mis}\bmath \beta)$$

\bigskip

The posterior distributions of $\bmath \beta$ and $\sigma$ ($p(\bmath\beta)$ and $p(\sigma)$)
are estimated from the corresponding regression model on the observed data.
...
$$p(\bmath y_{mis}\mid \bmath y_{obs},\bmath X, \bmath \beta, \bmath \sigma) p(\bmath \beta) p(\sigma)$$
...
Then $p(\bmath y_{mis}\mid \bmath y_{obs},\bmath X, \bmath \beta, \bmath \sigma)$
is a normal distribution and we can
\begin{itemize}
\item draw $\bmath{\tilde \beta}$ from $p(\bmath\beta)$
\item draw $\tilde\sigma$ from $p(\sigma)$
\item draw $\bmath y_{mis}$ from a normal distribution with mean (= expectation)
      $\bmath X \bmath{\tilde\beta}$ and variance $\tilde\sigma^2$
\end{itemize}
...
% \begin{frame}{\thesection. \insertsection}
% \framesubtitle{\thesection.\thesubsection. \insertsubsection}In \blue{Bayesian imputation} (the approach seen on slides
