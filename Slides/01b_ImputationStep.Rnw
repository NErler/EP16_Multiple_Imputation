...
$p(\bmath\beta, \sigma\mid \bmath y_{obs}\mid \bmath y_{obs}, \bmath X_{obs})$,
...
<<skylineplot, echo = F, fig.width = 8, fig.height = 3>>=
N <- 100
...
\begin{itemize}
\item Rubin's two \blue{ideas}:
      \begin{itemize}
      \item Missing values need to be represented by \blue{multiple imputed values}.
      \item A \blue{model is necessary} to obain good imputations.
      \end{itemize}
...
\item For the total variance, \blue{two sources of variation} need to be considered:
      \begin{itemize}
...
\item In PMM we need to consider
      ...
      \item the \blue{set of data} used to calculate/estmate the parameters.
      \end{itemize}
