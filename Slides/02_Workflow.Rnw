\begin{frame}[allowframebreaks=0.8]{Outline of Part II}
...
\begin{frame}[fragile]{Visualize the (missing) data}
\framesubtitle{Missing data pattern}
...
mdp <- mice::md.pattern(NHANES, plot = F)
head(mdp[, -c(7:13)]) # omit some columns to fit it on the slide
tail(mdp[, -c(7:13)])
@
<<echo = F>>=
options('width' = op)
...
JointAI::md_pattern(NHANES, print = F, yaxis_pars = list(cex.axis = 0.6))
@
\end{frame}
...
\begin{frame}[fragile]{Visualize the (missing) data}
\framesubtitle{Proportion of missing values \& complete cases}
<<NCC_calc>>=
...
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
\begin{itemize}
\item Do missing values in multiple variables always occur together?
      (e.g. blood measurements)
\item Are there structural missing values? (e.g. pregnancy status in men)
\item Are there patterns in the missing values?
      (e.g. only patients with hypertension have observations of HyperMed)
\item Are values missing by design?
\item Is the assumption of ignorable missingness (MAR or MCAR) justifiable?
\end{itemize}
\end{frame}
...
\begin{frame}{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
Auxiliary variables are variables that are not part of the analysis but can
...
\subsection{think about passive imputation, restrictions, post-processing}
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
\textbf{mice} has implemented many imputation methods, the
...
pred[, "HyperMed"] <- 0
imp <- mice(NHANES, method = meth, predictorMatrix = pred, printFlag = F)
save(imp, file = "workspaces/imp.RData")
@
<<impget, echo = F>>=
library(mice)
imp0 <- mice(NHANES, maxit = 0,
...
htmlwidgets::saveWidget(as_widget(htmlp),
                        file = file.path(getwd(), "figure", "squeeze_plot1.html"))
@
...
post["creat"] <- "imp[[j]][,i] <- squeeze(imp[[j]][,i], c(0, 100))"
imp2 <- update(imp, post = post, maxit = 20, seed = 123)
save(imp2, file = "workspaces/imp2.RData")
@
<<imp2get, echo = F>>=
post <- imp$post
post["creat"] <- "imp[[j]][,i] <- squeeze(imp[[j]][,i], c(0, 100))"
load("workspaces/imp2.Rdata")
@
...
% \begin{frame}[fragile]{\thesection. \insertsection}
% \framesubtitle{\thesection.\thesubsection. \insertsubsection}
% The function \Rfct{ifdo} may, for instance, come in handy when there are
% logical restrictions in the data.
%
% \bigskip
%
% In our data, subjects with \Rstring{SBP $\geq 140$} are classified as hypertensive.
% To use this information in the imputation we can specify:
<<postprocessing ifdo, cache = T, eval = F>>=
post["hypten"] <- "ifdo(SBP >= 140, levels(hypten)[2])"
@
% \end{frame}
...
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<convergence, fig.width = 10, fig.height = 5, fig.keep = "first">>=
...
\subsection{Fitting models with \texttt{mids} objects}
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
...
\subsection{Pooling results (\texttt{mira} $\rightarrow$ \texttt{mipo})}
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
...
\section{Some more functionality from mice}
...
imp2b <- update(imp2, post = post, maxit = 20, seed = 456) # same syntax as before, but different seed
save(imp2b, file = "workspaces/imp2b.RData")
@
<<ibindget, echo = F>>=
load("workspaces/imp2b.RData")
imp2combi <- ibind(imp2, imp2b)
imp2combi$m
@
\end{frame}
...
\only<1>{\includegraphics[width = \linewidth]{spss/MI01.JPG}}
\only<2>{\includegraphics[width = \linewidth]{spss/MI02.JPG}}
\only<3>{\includegraphics[width = \linewidth]{spss/MI03.JPG}}
\only<4>{\includegraphics[width = \linewidth]{spss/MI04.JPG}}
\end{column}
\begin{column}{0.5\linewidth}
\textsf{SPSS} lets you
\begin{itemize}
