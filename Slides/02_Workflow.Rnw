...
for (i in 1:ncol(NHANESnum)) {
  hist(NHANESnum[, i], nclass = 30, xlab = "", cex.main = 1.4, cex.axis = 1.1,
...
for (i in 1:ncol(NHANESfac)) {
  ...
  barplot(tab, cex.main = 1.4, cex.names = 1.2,
          main = paste0(names(NHANESfac)[i]," (",
...
<<mice example, eval = F, size = 'small'>>=
...
can be found \href{https://gist.github.com/NErler/0d00375da460dd33839b98faeee2fdab}{here}.

The function can be loaded into \textsf{R} using:
<<load gist, message = F, eval = F, size = 'small'>>=
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<probplot, warning = F, fig.width = 10, fig.height = 6, size = 'small'>>=
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<probplot_alc_print, eval = FALSE, size = 'small'>>=
probplot(imp2, formula = alc ~ race + gender)
@

<<probplot_alc, fig.width = 10, fig.height = 6, echo = FALSE>>=
probplot(imp2, formula = alc ~ race + gender, strip.text = element_text(size = 12))
@
\end{frame}
...
<<probplot_hypten_print, fig.width = 10, fig.height = 4.5, eval = FALSE>>=
probplot(imp2, formula = hypten ~ cut(age, quantile(age), include.lowest = T))
@

<<probplot_hypten, fig.width = 10, fig.height = 5, echo = FALSE>>=
probplot(imp2, formula = hypten ~ cut(age, quantile(age), include.lowest = T),
...
\item \Rarg{action}:
      ...
      \item \Rstring{"broad"}: \parbox[t]{0.6\linewidth}{
...
The function \Rfct{cbind.mids} allows to \blue{add columns} to a \Robj{mids} object.
...
\begin{block}{}
\centering\url{https://emcbiostatistics.shinyapps.io/MICourse_MICE}
\end{block}
or download the instructions and data for the practical from Canvas\\
(Files $>$ Principal documents $>$ Multiple Imputation $>$ Practical MICE).
