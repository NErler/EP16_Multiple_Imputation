...
JointAI::md_pattern(NHANES, print = FALSE, yaxis_pars = list(cex.axis = 0.5))
@
\end{frame}
...
We only use the correlation coefficient for binary variables in this
...
E.g.: to use \texttt{\hlstr{norm}} instead of \texttt{\hlstr{pmm}} for all
...
By \blue{default, all variables} (except the variable itself) \blue{are used} as predictor.\\
...
\blue{Important:}\\
A variable that has \blue{missing values needs to be imputed} in order to be used as
...
\blue{\ding{225}} Make sure all \blue{variables are coded correctly}, so that the chosen
...
\begin{itemize}
\item imputation models include an \blue{interaction terms} between incomplete variables
...
\blue{more data manipulations} and is not restricted to \Rfct{squeeze} and \Rfct{ifdo}.
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
Pooling with \Rfct{mice::pool} is available only for some types of model.
Generally, it should work for models for which the functions \Rfct{coef} and
\Rfct{vcov} can extract the (fixed effects) coefficients and variance-covariance
...
<<miceaddsfcts, eval = FALSE>>=
miceadds::micombine.chisquare(dk, df, display = TRUE, version = 1)
miceadds::micombine.F(values, df1, display = TRUE, version = 1)
@
...
<<miceaddsfcts2, eval = FALSE>>=
miceadds::micombine.cor(mi.res, variables = NULL, conf.level = 0.95,
                        method = "pearson", nested = FALSE, partial = NULL)
miceadds::micombine.cov(mi.res, variables = NULL, conf.level = 0.95,
                        nested = FALSE)
@
...
\subsection{Extracting and exporting imputed data}
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
The function \Rfct{complete} allows extraction of the imputed data from a \Robj{mids} object:
<<completedata, eval = FALSE>>=
mice::complete(x, action = 1, include = FALSE)
@

\begin{itemize}
\item \Robj{x}: the \Robj{mids} object
\item \Rarg{action}:
      \begin{itemize}
      \item \Rval{1}, \ldots, \Rval{m} (single imputed dataset)
      \item \Rstring{"long"}: long format (imputed data stacked vertically)
      \item \Rstring{"broad"}: wide format (imputed data combined horizontally; ordered by imputation)
      \item \Rstring{"repeated"}: (like \Rstring{"broad"}, but ordered by variable)
      \end{itemize}
\item \Rarg{include}: include the orginal data?\\
...
\begin{itemize}
\item select between linear regression imputation and pmm \blue{per} variable\\
      (only jointly for all variables)
\item uses \blue{only one donor} in predictive mean matching
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
In SPSS the list of models that can be pooled is available in the help
under
% Help $>$ Missing Values Option $>$ Multiple Imputation $>$Analyzign Multiple Imputation Data
