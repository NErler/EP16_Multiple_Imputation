...
\only<1>{
<<echo = F, fig.width = 5.25, fig.height = 3.5>>=
...
% the sampled coefficients, and \blue{variance determined by the variation of
...
\begin{itemize}
\item several variables with missing values (e.g., $p$)
\item several missing values in each of these variables
\item $m$ chains for each missing value
\end{itemize}
...
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
\only<1>{
<<echo = F, fig.width = 7, fig.height = 4.5>>=
library(ggplot2)
...
\only<4>{
<<echo = F, fig.width = 7.5, fig.height = 2.5>>=
...
\begin{block}{}
\centering\url{https://emcbiostatistics.shinyapps.io/Quiz_PartI}
\end{block}
