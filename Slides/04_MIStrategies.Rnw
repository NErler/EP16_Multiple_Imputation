...
\begin{itemize}
\item Gives an overview on the available \textsf{R} packages for missing data / imputation
\item Good point to start when searching a package with a particular functionality
\end{itemize}
