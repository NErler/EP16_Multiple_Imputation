...
\item what can be pooled depends on what is implemented. SPSS has  a restricted list.
In R we usually need a variance covariance matrix
\item set seeds to make results reproducible
\end{itemize}
\end{frame}
