...
Imputation methods implemented in the \textbf{mice} package:
...
mentioned <- paste0("mice.impute.",
                c("2l.lmer", "2l.norm", "2l.pan", "2lonly.mean", "passive",
...
micemeth <- sort(grep("mice.impute", getNamespaceExports("mice"), value = TRUE)) %>%
    cell_spec("latex", color = ifelse(. %in% used, "blue",
                                      ifelse(. %in% mentioned, "green!50!black", "black")))
...
@
\blue{Note:} \parbox[t]{0.8\linewidth}{Just because a method is implemented does
...
methtab2 <- matrix(data = c(miceaddsmeth, rep(NA, length(miceaddsmeth) %% 2)),
                   ncol = 2)
...
\item Convert the changed \Robj{data.frame} back to a \Robj{mids} object
      using the functions from the flow-diagram in the second practical
      (e.g., \Rfct{as.mids}, \Rfct{datalist2mids}, \Rfct{imputationList}, \ldots)
\end{itemize}
...
in the imputation models. Using a large number of predictor variables makes the
\blue{MAR assumption more plausible} (and, hence, reduces bias due to MNAR missingness)
and can \blue{reduce uncertainty} about the missing values.
...
from others, \ldots) however, \blue{in large datasets} (contain hundreds or thousands of
...
The amount of missingness that can be handeled \blue{depens on the information that
...
\textbf{Example:}
\begin{itemize}
\item In a set of $N = 50$ cases, 50\% missing values leave 25 cases to
      estimate the parameters of the predictive distribution.
\item In a large set of $N = 5000$ subjects, 50\% missing cases leave 2500
...
\begin{itemize}
\item the data to be imputed (and the context of the data),
\item the statistical method used for imputation, and
\item the software implementation used.
\end{itemize}

Moreover
\begin{itemize}
\item Never accept default settings of software blindly.
\item Question the plausibility of the MAR assumption. If it is doubtful,
...
\blue{Remember:}
\begin{itemize}
\item Use as much information as possible
      \begin{itemize}
      ...
      \end{itemize}
\item Avoid feedback from derived variables to their originals.
...
\item Consider the impact the visit sequence may have.
\item Choose an appropriate number of imputations.
\item Make sure the imputation algorithm has converged.
\item Use common sense when evaluating if the imputed values are plausible.
\end{itemize}
\end{frame}



\section{MICE and MI in the bigger picture}
\subsection{Other R packages that do imputation}
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
...
In Part III of this course (and the second practical) we have worked with some
\textsf{R} packages that perform imputation or provide functionality for
missing data other than \textbf{mice}.

\bigskip

Currently, there are \blue{\Sexpr{length(res2$missing)} packages} available on CRAN that
\blue{use the word ``missing''} in either the titel or description of the package,
\blue{\Sexpr{length(unique(c(res2$impute, res2$imputation)))}} that
\blue{use either ``impute'' or ``imputation''} and
\blue{\Sexpr{length(res2$incomplete)}} that \blue{use the word ``incomplete''}.

\bigskip

Not all of these packages perform imputation or are usefull for our purposes,
...
\item There are macros and additional packages available, e.g.e,
