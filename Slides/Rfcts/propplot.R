...
propplot <- function(x, formula, facet = "wrap", ncol = NULL, nrow = NULL, ...) {
  library(ggplot2)
  ...
  if (paste(formula[length(formula)]) != "1") {
    wvars <- gsub("[[:space:]]*\\|[[:print:]]*", "", paste(formula)[length(formula)])
    wvars <- attr(terms(as.formula(paste("~", wvars))), "term.labels")
    
    if (grepl("\\|", formula[length(formula)])) {
      svars <- gsub("[[:print:]]*\\|[[:space:]]*", "", paste(formula)[length(formula)])
  ...
  if (facet == "wrap")
    if (length(xnames) > 1) {
      ...
      if (is.null(wvars)) {
        print(p + facet_wrap('variable'))
      } else {
        print(p + facet_wrap(wvars, scales = "free", ncol = ncol, nrow = nrow))
      }
    }
