...
res_survreg <- list(mice_naive = res_mice_naive_wb,
                    mice = res_mice_wb,
                    JointAI = cbind(term = names(coef(JointAI_survreg)), 
                                    as.data.frame(summary(JointAI_survreg)$stat[
...
res_cox <- list(mice_naive = res_mice_naive_cox,
                ...
                JointAI = cbind(term = names(coef(JointAI_cox)),
                                as.data.frame(summary(JointAI_cox)$stat[
                                  names(coef(JointAI_cox)),
                                  c(1,3,4)]))
)
...
polyDF_cox <- data.frame(x = rep(c(0.5, length(res_cox) + .5)[c(1,2,2,1)],
                         ...
                         y = c(apply(log(summary(mod_cox)$conf.int[, 3:4]), 1, rep,
                                     each = 2)),
                         term = rep(names(coef(JointAI_cox)), each = 4)
)
...
polyDF_survreg <- data.frame(x = rep(c(0.5, length(res_survreg) + .5)[c(1,2,2,1)],
                             ...
                             y = c(apply(confint(mod_survreg), 1, rep,
                                     each = 2)),
                         term = rep(names(coef(JointAI_survreg)), each = 4)
)
