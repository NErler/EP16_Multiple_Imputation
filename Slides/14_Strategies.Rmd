...
## Number of Imputations
A \blue{suggested rule of thumb} [@White2011]:

\blue{number of imputed datasets} should be similar to the 
\blue{percentage of incomplete cases}


Since this percentage depends on the size of the dataset, the 
\blue{average percentage of missing values} per
variable could be used as an alternative. [@Buuren2012]
...
* especially in settings where the computational burden allows for it.
...
* Include auxiliary variables if
...
* Calculate \blue{summary scores} from multiple items referring to the same concept
...
* Convert the changed \Robj{data.frame} back to a \Robj{mids} object
      using the functions such as `as.mids()`{.R}, `miceadds::datalist2mids()`{.R},
...
In Part III of this course we have seen some
\textsf{R} packages that perform imputation or provide functionality for
missing data other than \textbf{mice}.

\bigskip

Currently, there are \blue{`r length(res2$missing)` packages} available on CRAN that
use the word \blue{``missing''} in either the title or description of the package,
\blue{`r length(unique(c(res2$impute, res2$imputation)))`} that
use either \blue{``impute'' or ``imputation''} and
\blue{`r length(res2$incomplete)`} that use the word \blue{``incomplete''}.
