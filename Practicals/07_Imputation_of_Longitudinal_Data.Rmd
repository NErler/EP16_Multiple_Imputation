...
in warning messages, and values are not being imptued.
...
This summary also has to be represented in the same number of valus for each 
...
### Task 4
We now need to combine the imputed basline data with the longitudinal variables
...
* Split the data by imputation number into a list of dataframes<br>
```{r, echo = FALSE, results = 'asis'}
...
the option `use_ggplot = TRUE` in `tracepot()`.",
     margin = 40)
```
:::
