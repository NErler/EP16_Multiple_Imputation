...
You need to load the **broom.mixed** package (i.e., `library("broom.mixed")`)
before pooling results from a mixed model.
...
**JointAI** provides the functions `lme_imp()` and `lmer_imp()`. 
The specification of the main model components is analogous to the function
`lme()` from the **nlme** package or `lmer()` from the **lme4** package.
...
tab <- rbind(lmm = c("linear mixed model", "continuous variables"),
             ...
             glmm_beta = c("beta mixed model", "continuous variables in [0, 1]"),
             glmm_logit = c("logistic mixed model", "factors with two levels"),
             glmm_probit = c("probit mixed model", "factors with two levels"),
             glmm_gamma_inverse = c("gamma mixed model (with inverse link)", "skewed continuous variables"),
             glmm_gamma_identity = c("gamma mixed model (with identity link)", "skewed continuous variables"),
             glmm_gamma_log = c("gamma mixed model (with log link)", "skewed continuous variables"),
             glmm_poisson_log = c("poisson mixed model (with log link)", "count variables"),
             glmm_poisson_identity = c("poisson mixed model (with identity link)", "count variables"),
             clmm = c("cumulative logit mixed model", "ordered factors with >2 levels"),
             mlogitmm = c("multinomial logit mixed model", "unordered factors with >2 levels")
)
...
This can be achieved using the **doFuture** package. For details see the
[vignette on MCMC settings](https://nerler.github.io/JointAI/articles/MCMCsettings.html#parallel-sampling-1).
:::
...
association of `day`. To do that, we can create a new dataset containing
information on an "average" subject, with different values for `day`.
...
ones specified in the argument `var` (a one-sided formula). The variables given
in `var` will range across the range of values of that variable encountered in
the data.
