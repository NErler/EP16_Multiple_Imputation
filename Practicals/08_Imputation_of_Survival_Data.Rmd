...
We first creat the event variable, and calculate the Nelson-Aalen estimator:
```{r}
library("mice")
library("survival")
...
Then we can continue as usual, by doing the set-up run and adpating the arguments
...
When we do the pooling, we get two warning messages.

The function `pool()` has an argument `dfcom`, refering to the degrees of freedom
from the complete data analysis, which is `NULL` by default.
In that case, mice will try to extract them from the first analysis. But this 
fails for a Cox PH model.

The first warning message (about the unknown column `df.residual`) can just be ignored.

The second warning means that, because the degrees of freedom could not be extracted
from the model, it will be assumed that this number is very large (i.e., that
the data is really large compared to the number of parameters that are estimated).
...
library("JointAI")
JointAIcox <- coxph_imp(Surv(time = time, event) ~ platelet + age + sex + chol + stage,
...
```{r, eval = runimps}
JointAIcox2 <- coxph_imp(Surv(time = time, event) ~ platelet + age + sex + chol + stage,
