...
```{r distr-solution, solution = TRUE, fig.widht = 9, fig.height = 9}
...
This means not only to check the `loggedEvents`, but also to make sure the
`method`, `predictorMatrix` and `visitSequence` that were used are what they
were supposed to be.
...
Find out if there is more information in the help of those two functions.
...
```{r distrplot-hint-1, eval = !static}
# You can use densityplot and probplot to get plots for all continuous and
# categorical variables.
# 
# To check all imputed values you can either get a summary of the "imp" element
...
```{r distrplot-solution, solution = TRUE, fig.width = 7, fig.height = 9}
densityplot(imp)
...
(Hoever, since the imputations seem fine overall, and there is little knowledge
gain in re-doing the previous steps, we will skip this repitition 
in this practical.)
...
When we are confident that the imputation was sucessfull, the imputed data can
...
Analyses performed on on `mids` objects will result in a `mira` object
...
The components of `imp$analyses` will each have the `class` depending on the
