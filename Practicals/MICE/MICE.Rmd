...
rmarkdown::render("Practicals/MICE/MICE.Rmd", 
                  ...
                  output_file = "Practical_MICE.nb.html")
...
if (names(rmarkdown::metadata$output) == "html_notebook") {
...
For this example, we will use the **NHANES** dataset. In the interactive version
...
**Note:**
If you want do this practical on your own computer, you need to load the data.
This can be done with the command `file.choose()` which opens the explorer and
allows you to navigate to the location of the file `NHANES.RData`. If you
...
Using `levels(NHANES$educ)` we can get the levels of the variable:

```{r levels-educ, exercise = TRUE}

```

```{r levels-educ-solution}
levels(NHANES$educ)
```
...
Now visualize and inspect the missing data pattern using one or more of the
...
```{r changeimpmod-setup, exercise.setup = "educrecode"}
imp0 <- mice(NHANES, maxit = 0, 
             defaultMethod = c("norm", 'logreg', 'polyreg', 'polr'))
```
...
argument. 
```{r predmat-setup, exercise.setup = "changeimpmod-setup"}
# imp0 <- mice(NHANES, maxit = 0, 
#              defaultMethod = c("norm", 'logreg', 'polyreg', 'polr'))
meth <- imp0$meth
meth["creat"] <- "pmm"
