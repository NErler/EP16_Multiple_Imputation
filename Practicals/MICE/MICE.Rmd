...
Let's check the missing data pattern specifically for those three variables:
...
```{r checkpolr-hint-1, include = !static}
...
```{r, echo = F, results = 'asis'}
if (static) {
  cat('<div id = "checkpolrhint" style="border:1px; border-style:solid; padding: 1em; border-color:#1F78B4">')
  cat('Note:')
} else {
...
```{r impnaiveVisSeq-hint-1, include = !static}
...
```{r convergence-solution, solution = TRUE, fig.width = 9, fig.height = 7, cache = TRUE}
...
```{r distrplot-hint-1, include = !static}
# You can use densityplot and probplot to get plots for all continuous and
# categorical variables.
# 
# To check all imputed values you can either get a summary of the "imp" element
