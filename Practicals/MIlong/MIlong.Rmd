...
b[sample(1:length(b), 30)] <- NA

pbclong$ascites <- a[match(pbclong$id, unique(pbclong$id))]
# pbclong$bili <- pbclong$bili[match(pbclong$id, unique(pbclong$id))]
...
JointAIlong <- lme_imp(albumin ~ day + sex + trt + age + ascites + 
                       ...
                       no_model = 'day', n.adapt = 5000,
                       data = pbclong, n.iter = 0, seed = 2019)
...
longcc <- with(pbclong,
               lmer(albumin ~ day + sex + trt + age + ascites +
                                     bili + copper +(day | id)))
