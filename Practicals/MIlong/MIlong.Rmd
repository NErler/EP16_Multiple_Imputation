...
pbclong <- subset(survival::pbcseq, select = c('id', 'trt', 'age', 'sex', 'day',
                                               'ascites', 'hepato', 'bili', 'chol',
                                               'albumin'))

set.seed(2019)
a <- factor(survival::pbc$ascites)
a[sample(1:length(a), 50)] <- NA

pbclong$ascites <- a[match(survival::pbcseq$id, survival::pbc$id)]
pbclong$copper <-
  survival::pbc$copper[match(survival::pbcseq$id, survival::pbc$id)]
...
In this practical we will work with data on from a trial on primary 
...
# using package nlme
lme(albumin ~ ns(day, df = 3) + sex + trt + age + ascites + bili + copper, random = ~day|id)


# using package lme4
lmer(albumin ~ ns(day, df = 3) + sex + trt + age + ascites + bili + copper + (day|id))
```
...
For such variables, the imputation methods 
...
meth_micelong[c("copper", "chol")] <- "2lonly.pmm"

pred_micelong[, "id"] <- -2
pred_micelong[, "day"] <- 2
pred_micelong[, "hepato"] <- 0
...
meth_micelong[c("hepato")] <- ""
meth_micelong[c('ascites', "chol", "copper")] <- "2lonly.pmm"
meth_micelong[c('ascites')]
...
micelong <- mice(pbclong, meth = meth_micelong, pred = pred_micelong,
                 maxit = 20, printFlag = T)

micelong_mira <- with(micelong, lme(albumin ~ ns(day, df = 3) + sex + trt +
                                      age + ascites + bili + chol,
                                    random = ~day|id)
)

# alternative:
micelong_mira <- with(micelong, lmer(albumin ~ ns(day, df = 3) + sex + trt +
                                       age + ascites + bili + chol +
                                       (day|id))
)
...
To analyse incomplete longitudinal data a linear mixed model the R package
...
```{r runJoitAIlong-solution, cache = TRUE, fig.width = 9, fig.height = 5, message = FALSE}
library(JointAI)
JointAIlong <- lme_imp(albumin ~ day + sex + trt + age + ascites +
                         bili + copper, random = ~ day|id,
                       models = c(copper = 'lognorm', bili = 'glmm_gamma'),
                       no_model = 'day', n.adapt = 500,
                       data = pbclong, n.iter = 1000, seed = 2019)
...
```{r}
newdf <- predDF(JointAIlong, var = 'day')
pred <- predict(JointAIlong, newdata = newdf)
...
ggplot() +
  geom_line(data = pbclong, aes(x = day, y = albumin, group = factor(id), color = id)) +
  geom_ribbon(data = pred$dat, aes(x = day, ymin = `2.5%`, ymax = `97.5%`), alpha = 0.5) +
  geom_line(data = pred$dat, aes(x = day, y = fit), lwd = 1.2)
...
JointAIlong2 <- glme_imp(hepato ~ ns(day, df = 3) + sex + trt + age + ascites +
                           bili + copper + chol, random = ~1|id, family = binomial(),
                         models = c(copper = 'lognorm', bili = 'glmm_gamma'),
                         no_model = 'day', data = pbclong, n.iter = 1000, seed = 2019)
...
<!-- ```{r, eval = static, echo = FALSE} -->
<!-- asis_output("#### Extra Task\\n") -->
<!-- ``` -->
<!-- Before, we assumed that the trajectories of `protime` had a linear evolution -->
<!-- over time and that a linear mixed model was appropriate. -->

<!-- A closer look at the data shows that the distribution of `protime` is a bit skewed, -->
<!-- so that a gamma distribution may be more fitting than a normal distribution. -->
<!-- We also want to explore if the trajectories are indeed linear. -->

<!-- ```{r, eval = static, echo = FALSE} -->
<!-- asis_output('<div style="border:2px; border-style:solid; padding: 1em; border-color:#446E9B">') -->
<!-- ``` -->

<!-- * Fit a gamma mixed model (with log-link) -->
<!-- * use a non-linear effect for `days` (e.g., a natural cubic spline with 3 degrees of freedom) -->


<!-- If you are satisfied by convergence and mixing of the chains, get the -->
<!-- model `summary()`. -->
<!-- ```{r runJoitAIlongnonlin, exercise = TRUE, fig.width = 9, fig.height = 5} -->
<!-- ``` -->
...
# mice -------------------------------------------------------------------------
micelong_mira <- with(micelong, lme(protime ~ day + sex + trt + age + 
                                      ascites + bili + chol + albumin,
                                    random = ~day|id)
)

micelong_mira <- with(micelong, lmer(protime ~ day + sex + trt + age + 
                                      ascites + bili + chol + albumin + (day|id))
)
...
longcc <- with(pbclong,
               lmer(protime ~ day + sex + trt + age + ascites +
                                     bili + chol + albumin +(day | id)))
options(contrasts = opt)


# combine the results ----------------------------------------------------------
longres <- list(
  mice = summary(pool(micelong_mira, dfcom = 99999),
...
ggplot(longresDF, aes(x = method, y = est)) +
  geom_point() +
  geom_errorbar(aes(min = lo, max = hi), width = 0.3) +
