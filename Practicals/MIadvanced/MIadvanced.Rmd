...
# NHANES$DBP[NHANES$DBP == 0] <- NA

# save(NHANES, file = "Practicals/MIadvanced/www/NHANES_2.RData")
...
# save(pbclong, file = "Practicals/MIadvanced/www/pbclong.RData")
...
# save(pbcdat, file = "Practicals/MIadvanced/www/pbcdat.RData")
load("www/NHANES_2.RData")
...
JointAIint <- lm_imp(weight ~ gender + bili + age * (chol + HDL) + height,
           data = NHANES, n.iter = 1000, inits = NULL,
           auxvars = c("DBP", "educ"))
t1 <- Sys.time()
...
t0 <- Sys.time()
set.seed(123)
jomointMCMC <- jomo.lm.MCMCchainC(weight ~ gender + bili + age * (chol + HDL) + height,
                    data = NHANES, nburn = 30000, output = 0)
t1 <- Sys.time()

set.seed(123)
jomoint <- jomo.lmC(weight ~ gender + bili + age * (chol + HDL) + height,
                   data = NHANES, nburn = 20000, 
                    nbetween = 1000, nimp = 5, output = 0)
...
par(mfrow = c(8, 9), mar = c(3,3,1,1), mgp = c(2, 0.6, 0))
...
asis_output('To get the **NHANES** data, load the file `NHANES_2.RData`.')
```
...
```{r runjomointMCMC-hint-2, include = !static, eval = !static}
jomointMCMC <- jomo.lm.MCMCchain(weight ~ gender + bili + age * (chol + HDL) + height,
                    data = NHANES, output = 0, nburn = 1000)
...
```{r runjomointMCMC-hint_2, eval = F, echo = static}
jomointMCMC <- jomo.lm.MCMCchain(weight ~ gender + bili + age * (chol + HDL) + height,
                    data = NHANES, output = 0, nburn = 30000)
```

```{r runjomointMCMC-hint_2b, include = static}
class(jomointMCMC)
names(jomointMCMC)
lapply(jomointMCMC, class)
```
...
<div style="border:1px; border-style:solid; padding: 1em; border-color:#D11450">
**Note:**<br>
In version 2.6-2 of **jomo** there is a bug in the sampling of the
covariance parameters, which will be resolved in the next version.
The objects `jomointMCMC` and `jomoint` (see later) used in the solution were
created using a preliminary fix of the bug.
</div>')
```


```{r jomointconv, exercise = TRUE, fig.width = 9, fig.height = 9}
# Note: In the current version of jomo there is a bug in the sampling of the
# covariance parameters, which will be resolved in the next version.
# The object "jomointMCMC" that you will work with from here on was created using a
# preliminary fix of the bug.
```
...
# Note: The 30000 iterations used in "jomointMCMC" took more than 5 minutes.
# For the purporse of this practical, use 1000 iterations or less.
# When you use "jomoint" in the code chunks below, this will be an object
# created with the fixed version, using a burnin of 20000 iterations.
